...
@ApiTags('group_account_message')
@Controller('group-account-message')
export class GroupAccountMessageController {
  constructor(
    private readonly groupAccountMessageService: GroupAccountMessageService,
  ) {}
  ...
  update(
    @Param('id') id: string,
    @Body() updateGroupAccountMessageDto: UpdateGroupAccountMessageDto,
  ) {
    return this.groupAccountMessageService.update(
      +id,
      updateGroupAccountMessageDto,
    );
  }
